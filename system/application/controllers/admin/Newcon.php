...
class Newcon extends Controller {
	...
	function index()
	{
		...
		{		
			//redirect("admin/request");
                    redirect("search_controller/alarm");
		}
		else
		{
	...
	function request()
	{
		...
		if($this->session->userdata('admin') !='')
		{
			...
			$data['request'] = $this->admin_model->booking_request();
			$this->load->view('admin/mist',$data);
		}
		else
		{
	...
	*/
	...
	function add_member_process()
	{
		...
		else
		{
			...
			$file_loacation = $config['upload_path']."/".$file_name;
            ...
            $this->load->model('book_model');
            $data=array();
            $data['result']=$this->book_model->selectMember();
